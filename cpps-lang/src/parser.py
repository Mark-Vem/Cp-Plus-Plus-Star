...
class Parser:
    ...
    def parse_stmt(self) -> Optional[ASTNode]:
        ...
        if t.type == TokenType.IDENTIFIER:
            ...
            if nxt and nxt.type == TokenType.DOUBLE_COLON and self._looks_like_var_decl():
                return self.parse_var_decl()
    ...
    def _looks_like_var_decl(self) -> bool:
        """
        Disambiguate `IDENT :: ...` at statement/primary level between:
          - a variable declaration: `name :: [mut] Type [= expr]`
          - a namespaced/static call or other expression:
            `ns::fn(args)`, `Type::method()`, `std::io::println(x)`

        Both forms start with `IDENT DOUBLE_COLON`, so a fixed lookahead
        of 2 tokens is not enough — bug found in practice: `fs::mkdir(...)`
        was always routed to `parse_var_decl`, which happily consumed
        "mkdir" as a bogus type name and then (via its no-`=` fallback)
        swallowed the call's argument list as an implicit initializer
        tuple, silently turning a real call into a dead variable
        declaration named `fs`.

        This performs pure lookahead (no token consumption / no parser
        state mutation) by walking the `IDENT (:: IDENT)*` chain that
        follows the leading `::`. If that chain is immediately followed
        by `(`, it's a call expression, not a var decl.
        """
        i = 1  # peek(0) is the leading name; peek(1) is the first `::`
        if not (self.peek(i) and self.peek(i).type == TokenType.DOUBLE_COLON):
            return False
        i += 1
        if self.peek(i) and self.peek(i).type == TokenType.KW_MUT:
            return True  # `name :: mut ...` is unambiguously a var decl
        if not (self.peek(i) and self.peek(i).type == TokenType.IDENTIFIER):
            return True  # doesn't look like a namespaced path either way
        i += 1
        while (self.peek(i) and self.peek(i).type == TokenType.DOUBLE_COLON and
               self.peek(i + 1) and self.peek(i + 1).type == TokenType.IDENTIFIER):
            i += 2
        nxt = self.peek(i)
        if nxt and nxt.type == TokenType.LPAREN:
            return False  # `name :: a::b::c(...)` -> namespaced call
        return True
    ...
    def parse_primary(self) -> Optional[ASTNode]:
        ...
        # Tried first (with full backtracking) because it shares the leading
        # '(' with tuple/paren expressions and can only be disambiguated by
        # scanning ahead for a trailing '->' after the matching ')'.
    ...
    def _try_parse_lambda(self) -> Optional['LambdaExpr']:
        """
        Speculatively parse a lambda expression starting at '(':

            (params) -> RetType ** { body }
            (params) -> RetType ** expr
            (params) -> RetType { body }        -- '**' is optional

        Uses full backtracking: if the lookahead does not resolve into a
        valid lambda signature (params, then ')', then '->'), the parser
        position is restored and the caller falls back to the ordinary
        parenthesized/tuple expression path.

        Returns:
            LambdaExpr on success, None if this is not a lambda.

        Note: `Parser.expect()` does not raise on mismatch — it records an
        error in `self.errors` and returns None, then callers keep going
        with placeholder values. That means a failed speculative parse can
        silently pollute `self.errors` even when we correctly detect the
        mismatch and roll back `self.pos`. We must snapshot and restore
        `self.errors` too, or callers see phantom "N parse errors" for
        perfectly valid code (e.g. `fs::mkdir(path, true)` briefly looks
        like a lambda parameter list to the speculative parser).
        """
        start = self.pos
        ...
        try:
            self.advance()  # consume '('
            params = self.parse_param_list()
            if not self.check(TokenType.RPAREN) or len(self.errors) > errors_start:
                self.pos = start
                del self.errors[errors_start:]
                return None
            self.advance()  # consume ')'
            ...
            self.match(TokenType.POW)  # optional '**' body delimiter
            if self.check(TokenType.LBRACE):
                self.advance()
                body: List[ASTNode] = self.parse_block()
            else:
                body = [ReturnStmt(self.parse_expr(), line)]
            if len(self.errors) > errors_start:
                self.pos = start
                del self.errors[errors_start:]
                return None
            return LambdaExpr(params, return_type, body, line)
